...
class Redirection extends React.Component {
    ...
    redirect(language, dataId, studyId, studyResultId) {
        let searchParams = new URLSearchParams([["lang", language], ["version", "cdst"], ["mode", "reg"], ["studyID", studyId], ["studyResultID", studyResultId], ["data", dataId]])
        ...
        if (process.env.NODE_ENV !== "development") {
            ...
            switch (this.props.cancelValue) {
                case "cancel_without_data":
                    ...
                    break;
                case "cancel_with_data":
                    config = new Blob([`save_all_data\n${Date.now()}\n${this.props.studyMetaTracker.studyTitle}\n${this.props.studyMetaTracker.studyUuid}`], {type: 'text/plain'})
                    searchParams = new URLSearchParams([["lang", this.props.studyMetaTracker.language], ["version", "cdst"], ["mode", "reg"], ["studyID", this.props.studyMetaTracker.studyId], ["studyResultID", this.props.studyMetaTracker.studyResultId], ["data", 2]])
                    this.props.handleCancelDialog();
